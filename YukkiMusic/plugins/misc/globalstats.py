...
async def stats_global(
    ...
    title = title.title()
    final = f"ᴍᴏsᴛ ᴘʟᴀʏᴇᴅ ᴛʀᴀᴄᴋ ᴏɴ​ {MUSIC_BOT_NAME}\n\n**ᴛɪᴛʟᴇ:** {title}\n\nᴘʟᴀʏᴇᴅ​** {co} **ᴛɪᴍᴇs​"
    not_sudo = [
...
**ᴀᴜᴛᴏ ʟᴇᴀᴠᴇ ᴍᴏᴅᴇ:** {ass}
**ᴄʟᴇᴀɴɪɴɢ ɪɴᴛᴇʀᴠᴀʟ:** {cm} ᴍɪɴᴜᴛᴇs

**ᴅᴜʀᴀᴛɪᴏɴ ʟɪᴍɪᴛ​:** {play_duration} ᴍɪɴᴜᴛᴇs
**ᴅᴏᴡɴʟᴏᴀᴅ ʟɪᴍɪᴛ:** {song} ᴍɪɴᴜᴛᴇs
**ᴘʟᴀʏʟɪsᴛ ʟɪᴍɪᴛ:** {playlist_limit}
**ᴘʟᴀʏʟɪsᴛ ᴘʟᴀʏɪɴɢ ʟɪᴍɪᴛ:** {fetch_playlist}"""
...
async def top_users_ten(client, CallbackQuery, _):
    ...
    for user, count in list_arranged.items():
        ...
        try:
            ...
            continue
        limit += 1
        msg += f"🥱`{user}` ᴘʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs ᴏɴ ʙᴏᴛ​.\n\n"
    if limit == 0:
        return await mystic.edit(_["tops_2"], reply_markup=upl)
...
async def top_ten_chats(client, CallbackQuery, _):
    ...
    for chat, count in list_arranged.items():
        ...
        try:
            ...
            continue
        limit += 1
        msg += f"🥱`{title}` ᴘʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs ᴏɴ ʙᴏᴛ​.\n\n"
    if limit == 0:
        return await mystic.edit(_["tops_2"], reply_markup=upl)
    if limit <= 10:
        limit = 10
...
async def top_fif_stats(client, CallbackQuery, _):
    ...
    for vidid, count in list_arranged.items():
        ...
        if vidid == "telegram":
            msg += f"🥱[ᴛᴇʟᴇɢʀᴀᴍ ꜰɪʟᴇs & ᴀᴜᴅɪᴏs​](https://t.me/telegram) ** ᴘʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs ᴏɴ ʙᴏᴛ​**\n\n"
        else:
            msg += f"🥱 [{title}](https://www.youtube.com/watch?v={vidid}) ** ᴘʟᴀʏᴇᴅ {count} ᴛɪᴍᴇs ᴏɴ ʙᴏᴛ​**\n\n"
    final = (
        _["gstats_3"].format(
